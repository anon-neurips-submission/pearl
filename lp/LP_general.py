...
# combined
def get_confusing_input_combined(net, orig_env, config_file, debug=False, value=True, policy=True,
                                 value_ceil=0, mu=0.5, device='cuda:0', writer=None):
    """
    ...
    # BINARY
    Agent_pos = {(i, j): 'Agent_pos(i{0},j{1})'.format(i, j) for (i, j) in
    ...
    A_fact = {(i, j): 'A_fact(i{0},j{1})'.format(i, j) for (i, j) in
              build_indicies_dictionary([shape[0][0], shape[0][1]])}
    A_sum = {(i, j): 'A_sum(i{0},j{1})'.format(i, j) for (i, j) in
             build_indicies_dictionary([shape[0][0], shape[0][1]])}
    ...
    if value:
        ...
        lin_expr_vars_lhs.append(X_15[(0)])
    ...
    for i in range(0, shape_out[0]):
        for j in range(0, shape_out[1]):
            ...
            problem.linear_constraints.add(
                ...
                senses=['E'],
                rhs=[0.0],
                names=[f"(Constraint_binary_2{i}_{j}"])
    ...
    for i in range(0, shape_out[0]):
        for j in range(0, shape_out[1]):
            ...
            problem.linear_constraints.add(
                ...
                senses=['E'],
                rhs=[0.0],
                names=[f"(Constraint_binary_2_2{i}_{j}"])
    ...
    for i in range(0, shape_out[0]):
        for j in range(0, shape_out[1]):
            ...
            problem.linear_constraints.add(
                ...
                senses=['E'],
                rhs=[0.0],
                names=[f"(Constraint_binary_3_2{i}_{j}"])
    ...
    for i in range(shape_out[0]):
        ...
        for j in range(shape_out[1]):
            ...
            problem.linear_constraints.add(
                ...
                senses=['E'],
                rhs=[0],
                names=[f"(Constraint_binary_4_1{i}_{j}"])
    ...
    for i in range(shape_out[0]):
        ...
        for j in range(shape_out[1]):
            ...
            problem.linear_constraints.add(
                ...
                senses=['E'],
                rhs=[0],
                names=[f"(Constraint_binary_5_1{i}{j}"])
    ...
    for i in range(shape_out[0]):
        ...
        for j in range(shape_out[1]):
            ...
            problem.linear_constraints.add(
                lin_expr=[cplex.SparsePair(lin_expr_vars_1 + lin_expr_vars_2 + lin_expr_vars_3,
                                           lin_expr_vals_1 + lin_expr_vals_2 + lin_expr_vals_3)],
                senses=['E'],
                rhs=[0],
                names=[f"(Constraint_binary_6_1{i}{j}"])
    ...
    jh = jensenshannon(pols_soft, [1/6, 1/6, 1/6, 1/6, 1/6, 1/6])
    ...
    return confusing_input_tensor, jh, output_valueS



# combined
def get_confusing_input_pool(net, orig_env, config_file, debug=False, value=True, policy=True,
                                 value_ceil=0, mu=0.5, device='cuda:0', writer=None):
    """
    ...
    problem.parameters.mip.pool.absgap.set(1e75)
    problem.parameters.mip.pool.relgap.set(1e75)
    problem.parameters.mip.limits.populate.set(10)
    ...
    for confusing_input_tensor in confusing_input_tensors:
        ...
        jh_list.append(jensenshannon(pols_soft, [1/6, 1/6, 1/6, 1/6, 1/6, 1/6]))
        output_valueS_list.append(ov_temp.squeeze(-1).tolist())


    return confusing_input_tensors, jh_list, output_valueS_list

def get_confusing_input_combined_trunks(net, orig_env, config_file, debug=False, value=True, policy=True):
    """
    ...
    # flatten --> dense
    X_5_1 = {(i): 'X_5(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[5]])}
    # dense --> relu
    X_6_1 = {(i): 'X_6(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[6]])}
    # relu --> dense
    X_7_1 = {(i): 'X_7(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[7]])}
    # dense --> relu
    X_8_1 = {(i): 'X_8(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[8]])}

    # relu --> policy x_10
    #      --> value x_13
    X_9_1 = {(i): 'X_9(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[9]])}


    if policy:
        # policy head
        # dense --> RELU
        X_10 = {(i): 'X_10(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[9]])}
        X_11 = {(i): 'X_11(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[10]])}
        X_12 = {(i): 'X_12(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[11]])}

    if value:

        # VALUE HEAD
        # (from X_9) dense --> relu
        X_13 = {(i): 'X_13(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[12]])}
        # relu --> Dense
        X_14 = {(i): 'X_14(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[13]])}
        # dense --> VALUE
        X_15 = {(i): 'X_15(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[14]])}
        # relu --> VALUE
        X_16 = {(i): 'X_16(i{0})'.format(i) for (i) in build_indicies_dictionary([shape[15]])}
    ...
    problem.variables.add(names=list(X_5_1.values()), lb=[0.0] * len(X_5), ub=[5.0] * len(X_5))

    problem.variables.add(names=list(X_6_1.values()), lb=[-5.0] * len(X_6), ub=[5.0] * len(X_6))

    problem.variables.add(names=list(X_7_1.values()), lb=[0.0] * len(X_7), ub=[5.0] * len(X_7))

    problem.variables.add(names=list(X_8_1.values()), lb=[-5.0] * len(X_8), ub=[5.0] * len(X_8))

    problem.variables.add(names=list(X_9_1.values()), lb=[0.0] * len(X_9), ub=[5.0] * len(X_9))
    ...
    problem.objective.set_linear(list(zip(list(X_7.values()), [1.0] * len(X_7))))
    problem.objective.set_linear(list(zip(list(X_9.values()), [1.0] * len(X_9))))
    problem.objective.set_linear(list(zip(list(X_7_1.values()), [1.0] * len(X_7))))
    problem.objective.set_linear(list(zip(list(X_9_1.values()), [1.0] * len(X_9))))

    if policy:
        problem.objective.set_linear(list(zip(list(X_11.values()), [1.0] * len(X_11))))

    if value:
        problem.objective.set_linear(list(zip(list(X_14.values()), [1.0] * len(X_13))))
        problem.objective.set_linear(list(zip(list(X_16.values()), [1.0] * len(X_15))))
    ...
    """CONSTRAINTS (ReLU_3_1) """
    #  X_2 >= X_1
    X_out = X_7_1  # this is the output of the Relu
    X_in = X_6_1  # this is the input to the Relu
    shape_ = shape[7]
    for i in range(shape_):
        lin_expr_vars_lhs = [X_out[(i)]]
        lin_expr_vals_lhs = [1.0] * len(lin_expr_vars_lhs)
    ...
    """CONSTRAINTS (ReLU_4_1) """
    #  X_2 >= X_1
    X_out = X_9_1  # this is the output of the Relu
    X_in = X_8_1  # this is the input to the Relu
    shape_ = shape[9]
    for i in range(shape_):
        lin_expr_vars_lhs = [X_out[(i)]]
        lin_expr_vals_lhs = [1.0] * len(lin_expr_vars_lhs)
    ...
    # b_dense_arr = np.ones(shape=(256))
    # make the biases an array
    X_out = X_6_1  # this is the output of the FC layer
    X_in = X_5_1
    ...
    W_dense_arr = net_params[10]
    b_dense_arr = net_params[11]  # make the biases an array
    ...
    X_out = X_8_1  # this is the output of the FC layer
    X_in = X_7_1
    ...
    if value:
        ...
        W_dense_arr = net_params[16]
        b_dense_arr = net_params[17]
        # (torch here)
        # W_dense_arr = np.ones(shape=(128,128))
    ...
    X_out = X_5_1  # this is the output of the Flatten
    X_in = X_4  # this is the input to the Flatten
    shape_ = shape[5]  # shape of the output of the flatten  layer
    shape_in = shape[4]  # shape of the input  of the flatten  layer
    # ini
    ...
    if policy:
        ...
        W_dense_arr = net_params[14]  # make the weights an array
        b_dense_arr = net_params[15]

        X_out = X_12
        X_in = X_11

        shape_ = shape[11]  # shape of the output of the FC  layer
        shape_in = shape[10]  # shape of the input  of the FC  layer
    ...
    if value:
        ...
        lin_expr_vars_lhs.append(X_16[(0)])
        lin_expr_vals_lhs.append(1.0)
    ...
    problem.parameters.randomseed(random.seed())
    problem.write(filename=f'constraint_check_{postfix}.lp')
    problem.solve()
    ...
    output_probabilities, output_value = net.forward(confusing_input_tensor)
    valss = torch.sigmoid(output_value)
    output_value = valss.squeeze(-1).tolist()
    ...
    print('pre-softmax', output_probabilities)
    print('pre-sigmoid', output_value)
    # BEFORE SOFTMAX
    # 0.018, 0.0623, -0.0818, -0.049, 0.0078, 0.003
    print('output_probabilities = ', pols_soft)
    print('output_value = ', output_value)
