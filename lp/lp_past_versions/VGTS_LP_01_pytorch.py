...
config_file = 'autograph/play/config/mine_woodfactory/simple_eval_envs/small_eval_simple.json'
...
CHECKPOINT_PATH = 'checkpoints/factHome_simple80_copy'
...
# X_0_bias =  {(i): 'X_0_b(i{0})'.format(i) for (i) in build_indicies_dictionary(shape[1])}
X_1 =  {(i,j,k): 'X_0(i{0},j{1},k{2})'.format(i,j,k) for (i,j,k) in build_indicies_dictionary((32, 6, 6))}


X_2 =  {(i,j,k): 'X_1(i{0},j{1},k{2})'.format(i,j,k) for (i,j,k) in build_indicies_dictionary([shape[2][0],shape[2][1], shape[2][2]])}
# X_2_bias =  {(i): 'X_1_o(i{0})'.format(i) for (i) in build_indicies_dictionary(shape[3])}
X_3 =  {(i,j,k): 'X_0(i{0},j{1},k{2})'.format(i,j,k) for (i,j,k) in build_indicies_dictionary((64, 6, 6))}

X_4 =  {(i): 'X_4(i{0})'.format(i) for (i) in build_indicies_dictionary(shape[4])}
...
X_15 = {(i): 'X_15(i{0})'.format(i) for (i) in build_indicies_dictionary(shape[15])}
...
#
# below needs to be pulled out from the pytorch net (torch here)
W_conv_arr = net_params[lay]

b_conv_arr = net_params[bias_lay]
...
for nnn in range((number_of_filters)):
    ...
    for i in range((shape_out[0])):
        for j in range((shape_out[1])):
            ...
            print(f'X_in is {X_out}')

            for iii in range(input_i[0][0],input_i[0][1]):
...
# get conv filters parameters:
#strides = net.layers[lay].strides[0]
strides = 1
pool_size_W = W_conv_arr.shape[0]
pool_size_H = W_conv_arr.shape[1]
pool_size_D = W_conv_arr.shape[2]
...
output_probabilities = net(X_0_1d.reshape(1,28,28,1))[0]
