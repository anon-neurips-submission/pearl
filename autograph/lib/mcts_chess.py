...
class MCTS(Generic[T]):
    def __init__(self, num_actions: int,
        ...
        self.set_of_all_actions = set(range(0, 4672))
    ...
    def pick_action(self, state: T, root_state: bool) -> int:
        ...
        action_raw = np.array(score)

        possible_moves = state[2]

        sum_of_ones = 0
        sum_of_ones += np.sum(action_raw[possible_moves])
        impossible_actions = list(self.set_of_all_actions - set(possible_moves))
        ...
        # add this early return to deal with divide by zero error on above line
        elif len(possible_moves) == 0:
            return int(np.argmax(action_raw))
        actions_legal = action_raw[possible_moves]
    ...
    def find_leaf(self, env: go_env, cur_state: T) -> Tuple[BackupTrace, T, bool]:
        ...
        while not done and not self.is_leaf(cur_state):
            ...
            if 'you want to brick your output' and ants:
                print("TREE STEP ", i)
                i+=1
            ...
            if done or next_state[1] in seenstates:
                done = True

                self.sigma[cur_state[1]][action] = 0
            else:
                seenstates.add(cur_state[1])
    ...
    def find_leaf_batch(self, env: go_env, num_leaves: int, start_state: T) \
        ...
        for i in range(num_leaves):
            ...
            if done:
                ...
                if end_state[1] not in will_create:
                    will_create.add(end_state[1])
    ...
    def is_leaf(self, state: T) -> bool:
        ...
        """
        return state[1] not in self.probs
    ...
    def get_policy_value(self, state: T, tau: float = 1) -> Tuple[List[float], List[float]]:
        ...
        if tau == 0:  # Set the best action to 1, others at 0
            ...
            total = sum(counts)
            probs = [(count + 1/10) / (total + 1) for count in counts]
