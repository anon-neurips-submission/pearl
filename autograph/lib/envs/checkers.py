...
class raw_env(AECEnv):
    ...
    def observe(self, agent):
        ...
        if agent == "player_1":
            ...
            obs = np.roll(obs, 2, axis=0)
            # Rotate board to place black pieces at bottom
            obs = np.rot90(obs, 2, axes=(1, 2))
    ...
    def reset(self):
        ...
        self.next_legal_moves = self.legal_moves()
        self.game_step = 0
    ...
    def step(self, action):
        ...
        if winner == "black":
            ...
            ret_rewards = [1,-1] if self.agent_selection == "player_0" else [-1,1]
            print("uppercase team won (black player0)")
        elif winner == "white":
            self.winner = 1
            self.rewards[self.agents[0]] = -1
            self.rewards[self.agents[1]] = 1
            ret_rewards = [1,-1] if self.agent_selection == "player_1" else [-1,1]
            print("lowercase team won (white player1)")

        all_done = winner is not None
        self.dones[self.agent_order[0]] = all_done
        self.dones[self.agent_order[1]] = all_done

        self._last_obs['action_mask'] = np.zeros(256, dtype=int)
        ...
        return self._last_obs, ret_rewards, all_done, dict()


    def render(self, mode="human"):
        print("####################")
        print(f"# Turn {self.game_step}: {'UPPERCASE' if self.last_turn == 'white' else 'lowercase'}")
        board = self.ch.flat_board()
        pieces = {
            1: "M",
            2: "K",
    ...
    def save_state(self):
        ...
        return self.ch.save_state(), self.agent_selection, self.observation, \
               self.last_turn, self.rewards, self.dones, self.game_step

    def load_state(self, state):
        self.ch.restore_state(state[0])
        self.agent_selection = state[1]
        self.last_turn = state[3]
        self.rewards = state[4]
        self.dones = state[5]
        self.observation = state[2]
        self.next_legal_moves = self.legal_moves()
        self.game_step = state[6]



    # where board is an np.array. 0's for empty. 1's for black. -1's for white.
    def reset_with_board(self, board, turn=0):
        self.ch = CheckersRules(board=board)
        self.agents = self.possible_agents[:]
        self.agent_selection = self.agent_order[turn]
        self.num_moves = 0
        self.agents = self.possible_agents[:]
        self.agent_order = list(self.agents)
        self.infos = {name: {} for name in self.agents}
    ...
    def load_state_from_tensor(self, confusing_tensor):
        array_of_board = confusing_tensor.cpu().detach().numpy().reshape((4, self._N, self._N))
        print(array_of_board)
        my_men = array_of_board[0]
        my_kings = array_of_board[1]
        your_men = array_of_board[2]
        your_kings = array_of_board[3]

        indexes_of_my_men = my_men.nonzero()
        indexes_of_my_kings = my_kings.nonzero()

        indexes_of_your_men = your_men.nonzero()
        indexes_of_your_kings = your_kings.nonzero()

        print(indexes_of_my_men)
        print(indexes_of_my_kings)
        print(indexes_of_your_men)
        print(indexes_of_your_kings)
        '''__00__01__02__03
           04__05__06__07__
           __08__09__10__11
           12__13__14__15__
        ...
        loaded_board = self.ch.empty_board()
        print(len(indexes_of_my_men[0]))
        # reference to global conversion dictionary
        if len(indexes_of_my_men[0]) > 0:
            loaded_board['black']['men'] = set(self.move64_32[pos2sq(x,y)] for x,y in zip(indexes_of_my_men[0], indexes_of_my_men[1]))
        if len(indexes_of_my_kings[0]) > 0:
            loaded_board['black']['kings'] = set(self.move64_32[pos2sq(x,y)] for x, y in zip(indexes_of_my_kings[0], indexes_of_my_kings[1]))

        if len(indexes_of_your_men[0]) > 0:
            loaded_board['white']['men'] = set(self.move64_32[pos2sq(x,y)] for x, y in zip(indexes_of_your_men[0], indexes_of_your_men[1]))
        if len(indexes_of_your_kings[0]) > 0:
            loaded_board['white']['kings'] = set(self.move64_32[pos2sq(x,y)] for x, y in zip(indexes_of_your_kings[0], indexes_of_your_kings[1]))
        return self.reset_with_board(loaded_board)
