...
class OrderEnforcingWrapper(BaseWrapper):
    ...
    * warn on calling step after environment is done
    ...
    def save_state(self):
        return self._has_reset, self._has_updated, self._has_rendered, super().save_state()

    def load_state(self, state):
        self._has_reset = state[0]
        self._has_updated = state[1]
        self._has_rendered = state[2]
        return super().load_state(state[3])
