...
from lp.LP_general_chessEnv import get_confusing_input_pool as gcip_chess
from lp.LP_general_checkers import get_confusing_input_pool as gcip_checkers

from autograph.lib.shaping import AutShapingWrapper
import cplex
from autograph.lib.envs.go.go_env import env as goEnv
from autograph.lib.envs.chess import chess_utils
from autograph.lib.envs.checkers import env as checkersEnv


# play steps happen here.
def run_episode_generic_lp_checkers(env: checkersEnv,
    ...
    if not lp_gen:
        ...
        next_state = env.observe('player_0')
    ...
    while not done:
        ...
        summm = action_raw.sum()
        if summm != 1 and summm != 0:
            action_raw /= summm

        if summm == 0:
            action_raw[list(range(0, 256))] = 1/256

        action_selected1 = action_selector(numpy.array([action_raw]))

        action_selected = action_selected1[0]
        #TAKE THE ACTION NOW
        done = env.env.env.env.env.dones['player_0'] or env.env.env.env.env.dones['player_1']
        if not done:
            next_state, rewards, done, info = env.step(action_selected)
        else:
            # if already done; curr player lost
            info = dict()
            next_state = state
            rewards = [-1, 1]
        if render:
...
def run_episode_generic_lp_chess(env,
                           action_value_generator: Callable[[Any, int], Tuple[List[float], float]],
                           max_length: int,
                           max_len_reward: Union[int, None],
                           action_selector: ptan.actions.ActionSelector = ptan.actions.ArgmaxActionSelector(),
    ...
    :return: A list of steps taken, and the value estimate of the final state
    """
    set_of_all_actions = set(range(0, 4672))
    done = False
    if not lp_gen:
        next_state = env.reset()
    else:
        next_state = env.observe(None)
        next_legal_moves = chess_utils.legal_moves(env.env.env.env.env.board)
        next_state = (next_state['observation'], env.env.env.env.env.get_fen(), next_legal_moves)
        print('LPSTATE IS BEING USED: ', next_state[1])
    ...
    while not done:
        ...
        possible_moves = state[2]

        sum_of_ones = 0
        sum_of_ones += numpy.sum(action_raw[possible_moves])
        impossible_actions = list(set_of_all_actions - set(possible_moves))
        action_raw[impossible_actions] = 0

        # if no actions have scores; then scale available actions to all the the same small percent (0.02)
        if sum_of_ones == 0 and len(possible_moves) != 0:
            action_raw[possible_moves] = 1 / len(possible_moves)

        ar_sum = action_raw.sum()
        if ar_sum != 1 and ar_sum != 0:
            action_raw /= action_raw.sum()

        if ar_sum == 0:
            action_raw += (1/len(action_raw))
        ...
        if render:
            if 'player_moved' in info and 'move' in info:
                if not info['move']:
                    print(info['player_moved'] + ' in checkmate, other player wins')
                    print(rewards)
                else:
                    print(info['player_moved'] + ' with ' + str(info['move']))
            env.render()
        ...
        other_value = 0
        # Get the next action/value pair (now instead of beginning of loop so that we easily have access to next value)
        env_saved = env.save_state()
        if done:  # because chess ends on the losers turn (when loser is in checkmate) switch who gets what value
            next_value = rewards[0]
            other_value = rewards[1]
        else:
            next_action_raw, next_value = action_value_generator(next_state, 1 if lp_gen else 0)
            if length >= max_length:
                ...
                if max_len_reward is not None:
                    next_value = max_len_reward
        env.load_state(env_saved)
        trace_state = state[0]
        trace_next_state = state[0]
        ...
        if length % 2 == 1:
            ...
            if done:
                trace.append(
                    TraceStep(trace_state, value, action_raw, action_selected, trace_state, next_value, rewards[0],
                              info, done_from_env))
                ...
                # append to black stone player traces
                trace.append(
                    TraceStep(trace_state, value, action_raw, action_selected, trace_next_state, next_value, rewards[0],
                              info, done_from_env))
        else:
            if done:
                trace_b.append(
...
# play steps happen here.
def run_episode_generic_lp_go(env: goEnv,
                           action_value_generator: Callable[[Any, int], Tuple[List[float], float]],
                           max_length: int,
                           max_len_reward: Union[int, None],
                           action_selector: ptan.actions.ActionSelector = ptan.actions.ArgmaxActionSelector(),
...
# play steps happen here.
def run_episode_generic_eval_go(env: goEnv,
                           action_value_generator: Callable[[Any, int], Tuple[List[float], float]],
                           max_length: int,
                           max_len_reward: Union[int, None],
                           action_selector: ptan.actions.ActionSelector = ptan.actions.ArgmaxActionSelector(),
...
def run_episode_generic_eval_chess(env: goEnv,
                           action_value_generator: Callable[[Any, int], Tuple[List[float], float]],
                           max_length: int,
                           max_len_reward: Union[int, None],
                           action_selector: ptan.actions.ActionSelector = ptan.actions.ArgmaxActionSelector(),
    ...
    # use net  1 if lp gen state else use net 0 (random net) this is not eval
    next_action_raw, next_value = \
        action_value_generator(next_state, 0 if env.env.env.env.env.agent_selection == 'player_0' else 1)
    env.load_state(env_state)

    # so we save our traces in the right trace lists
    length = 0 if env.env.env.env.env.agent_selection == 'player_0' else 1
    ...
    while not done:
        ...
        if action_raw.sum() != 1 and action_raw.sum() != 0:
            action_raw /= action_raw.sum()
...
def run_episode_generic_eval_checkers(env: checkersEnv,
                           action_value_generator: Callable[[Any, int], Tuple[List[float], float]],
                           max_length: int,
                           max_len_reward: Union[int, None],
                           action_selector: ptan.actions.ActionSelector = ptan.actions.ArgmaxActionSelector(),
...
class RenderEnvAndReturnWrapperPool:
    ...
    def __init__(self, func, lp_gen=False, config=None, device='cuda:0', writer=None,
                 cur_step=None, cur_round=None, lp_state_counts=None, lp_every_x_states=3, lp_states_in_pool=5, game="go"):
        self.func = func
        self.lp_gen = lp_gen
        ...
        self.lp_every_x_states = lp_every_x_states
        self.lp_state_queue = collections.deque()
        self.lp_states_in_pool=lp_states_in_pool
        self.game = game
    def __call__(self, env: AutShapingWrapper, nets, **kwargs):
        ...
        using_lp = False
        if self.lp_gen:
            if (self.cur_round_num % self.lp_every_x_states == 0): #TODO: SET lp_every_x_states TO TRUE for all LP
                using_lp = True
            else:
                using_lp = False
                ...
                try:
                    if self.game == "go":
                        state_tensors, jensens, output_values = get_confusing_input_pool(
                            ...
                            device=self.device, writer=self.writer, lp_states_in_pool=self.lp_states_in_pool)
                    elif self.game == "chess":
                        state_tensors, jensens, output_values = gcip_chess(
                            nets[0], env.env, self.config_file,
                            debug=False, policy=True, value=True,
                            device=self.device, writer=self.writer, lp_states_in_pool=self.lp_states_in_pool)
                    elif self.game == "checkers":
                        state_tensors, jensens, output_values = gcip_checkers(
                            nets[0], env.env, self.config_file,
                            debug=False, policy=True, value=True,
                            device=self.device, writer=self.writer, lp_states_in_pool=self.lp_states_in_pool)
                    else:
                        raise EnvironmentError("Are you using the right environment? currently supporting chess, go, checkers")
...
# MODIFIED FUNCTION TO OPTIONALLY GENERATE LP CONFUSING INPUT TENSOR IN ONE OF THE WORKERS (WORKER 0).
@contextmanager
def get_parallel_queue(num_processes, episode_runner, env, nets, device, lp_gen=False, config_file=None,
                       all_work_lp=False,
                       num_rounds=0, global_step=0, lp_every_x_states=3, lp_states_in_pool=5, game="go", **kwargs):
    """
    ...
    for i in range(num_processes):
        ...
        if render or all_work_lp:
            this_episode_runner = RenderEnvAndReturnWrapperPool(episode_runner,
                                                            ...
                                                            lp_every_x_states=lp_every_x_states,
                                                            lp_states_in_pool=lp_states_in_pool,
                                                            game=game)
...
class RandomReplayTrainingLoop:
    ...
    def __init__(self, discount: float, replay_buffer_len: int, min_trace_to_train: int, train_rounds: int,
                 obs_processor: Callable, writer: SummaryWriter, device, log_LP=False, game="go"):
        self.device = device
    ...
    def __call__(self, sim_round_queue, loss_functions, optimizers):
        ...
        if lp_generated:
            if self.game == "go" or self.game == "checkers":
                lp_state = sha1(trace[0][0])
            elif self.game == "chess":
                lp_state = trace[0][1]
            else:
                raise EnvironmentError("Which environment are you using?")
        ...
        traces = [trace, trace_b]
        # train on traces for both teams separately; don't train on games where we don't get a reward
        for trace, last_val in zip(traces, last_vals):
            if last_val != 0:
