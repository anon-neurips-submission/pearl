...
from autograph.lib.running_ensemble import get_parallel_queue, RandomReplayTrainingLoop, run_episode_generic_lp, run_episode_generic
...
def run_mcts_aut_episode_ensemble(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, c_puct, c_aut,
                         num_batches, batch_size, stats: AutStats, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None,c_sigma=1, lp_gen=False,
    ...
    #TODO: should both nets be working in state evaluator? Like LP for LP states and random for random states?
    ...
    mcts = MCTSAut(env.action_space.n, None, state_evaluator, None, c_puct=c_puct,
                   aut_stats=stats, c_aut=c_aut, c_sigma=c_sigma, **kwargs)

    def action_value_generator(state):
        mcts.mcts_batch(env, state, num_batches, batch_size, lp_gen)
...
def run_aut_episode_ensemble_deepQ(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, stats: AutStats, train_state_rewriter: Callable[[Any], torch.Tensor],
...
def run_aut_episode_ensemble_deepQ_noLP(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, stats: AutStats, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None,lp_gen=False,
                         jensen_shannon=None, output_value=None, **kwargs) \
...
def run_aut_episode(nets: torch.nn.Module, env: AutShapingWrapper, max_length: int, max_len_reward: Optional[int],
                    device,train_state_rewriter: Callable[[Any], torch.Tensor], stats: AutStats,
...
episode_runners = {
    "mcts_aut_episode_ensemble": run_mcts_aut_episode_ensemble,
    "no_mcts_aut_episode_ensemble": run_aut_episode_ensemble_deepQ,
    "no_mcts_aut_episode_ensemble_noLP": run_aut_episode_ensemble_deepQ_noLP,
...
def run():
    ...
    for tr_idx in range(0, NUM_RUNS):
        ...
        net_lp = cman.load("net_lp", train_net_creator(orig_env, **NETWORK_PARAMS),
                           CombinedLoadHandler(StateDictLoadHandler(), InitZeroLoadHandler())).to(DEVICE)
        ...
        both_nets = False
        if EPISODE_RUNNER_TYPE == "mcts_aut_episode_min_val" \
                or EPISODE_RUNNER_TYPE == "mcts_aut_episode_prob" \
                or EPISODE_RUNNER_TYPE == "mcts_aut_episode_combined":
            lp_gen = True
        elif EPISODE_RUNNER_TYPE == "mcts_aut_episode_ensemble" \
                or EPISODE_RUNNER_TYPE == "no_mcts_aut_episode_ensemble":
            lp_gen = True
            both_nets = True
        else:
            lp_gen = False
        ...
        with get_parallel_queue(num_processes=NUM_PROCESSES, episode_runner=episode_runners[EPISODE_RUNNER_TYPE],
                                ...
                                state_observer=None, device=DEVICE, stats=wrapped_aut_stats,
