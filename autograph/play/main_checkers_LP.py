...
from autograph.lib.mcts_checkers import MCTS
from autograph.lib.running_ensemble_checkers import get_parallel_queue, RandomReplayTrainingLoop, run_episode_generic_lp, run_episode_generic
from autograph.lib.shaping import AutShapingWrapper
from autograph.lib.util import element_add
from autograph.lib.util.checkpoint_manager import CheckpointManager, StateDictLoadHandler, PickleLoadHandler, \
    TransplantCheckpointManager, CombinedLoadHandler, InitZeroLoadHandler
...
from autograph.net.mine_constructors import minenet_v1, mine_obs_rewriter_creator, minernd_v1, \
    mine_mazenet_v1, mine_mazenet_v1_go, mine_obs_go_rewriter_creator, mine_mazenet_v1_checkers, \
    mine_obs_checkers_rewriter_creator
from autograph.net.misc_constructors import gym_make, no_op_cur_make, basic_net, no_op_make
from lp.LP_general_checkers import get_confusing_input_pool
...
if __name__ == '__main__':
    ...
    if EPISODE_RUNNER_TYPE == "checkers_mcts_LP" or EPISODE_RUNNER_TYPE == "checkers_mcts":
        LOSS_FUNC = "MCTS"
...
def run_checkers_mcts_LP(nets: List[torch.nn.Module], env: checkersEnv, max_length: int,
    ...
    # 256 is num actions
    mcts = MCTS(256, None, state_evaluator, None, **kwargs)
...
def run_checkers_ppo(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None,lp_gen=False,
...
def run_checkers_ppo_LP(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None,lp_gen=False,
                         jensen_shannon=None, output_value=None, **kwargs) \
...
episode_runners = {
    "checkers_ppo_LP": run_checkers_ppo_LP,
    "checkers_ppo": run_checkers_ppo,
    "checkers_mcts": run_checkers_mcts,
    "checkers_mcts_LP": run_checkers_mcts_LP,
}
...
def run():
    ...
    for tr_idx in range(0, NUM_RUNS):
        ...
        if EPISODE_RUNNER_TYPE == "checkers_ppo_LP" or EPISODE_RUNNER_TYPE == "checkers_mcts_LP":
            lp_gen = True
        else:
            lp_gen = False
