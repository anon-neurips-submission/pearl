...
from autograph.lib.running import get_parallel_queue, RandomReplayTrainingLoop, run_episode_generic_lp, run_episode_generic
...
if __name__ == '__main__':
    ...
    p.add_argument("--randomLate", dest="random_late", action="store_true")
    ...
    RANDOM_LATE = args.get("random_late", False)
    NUM_PROCESSES = int(args["workers"])
    DEVICE = torch.device(args["device"])

def run_mcts_aut_episode_lp(net: torch.nn.Module, env: AutShapingWrapper, max_length: int,
                         ...
                         state_observer: Callable[[Any], None] = None,c_sigma=1, c_intrins=1, lp_gen=False,
    ...
    def state_evaluator(states, lp_genn=None):
...
def run_mcts_aut_episode(net: torch.nn.Module, env: AutShapingWrapper, max_length: int,
                         ...
                         device, c_puct, c_aut,
                         num_batches, batch_size, stats: AutStats, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None, c_sigma=1, c_intrins=1, lp_gen=False,  **kwargs) \
        -> Tuple[List[TraceStep], float]:
    """
    Run an episode using MCTS with curiosity as the action selection
    :param net: The policy/value network
...
episode_runners = {
    ...
    "mcts_aut_episode_min_val": run_mcts_aut_episode_lp,
    "mcts_aut_episode_prob": run_mcts_aut_episode_lp,
    "mcts_aut_episode_combined": run_mcts_aut_episode_lp,
...
def run():
    ...
    for tr_idx in range(0, NUM_RUNS):
        ...
        with get_parallel_queue(num_processes=NUM_PROCESSES, episode_runner=episode_runners[EPISODE_RUNNER_TYPE],
                                ...
                                all_work_lp=ALL_WORK_LP, num_rounds=train_loop.num_rounds, global_step=RANDOM_LATE,
...
'''
        while True:
            with get_parallel_queue(num_processes=NUM_PROCESSES, episode_runner=episode_runners[EPISODE_RUNNER_TYPE],
                                    ...
                                    lp_gen=lp_gen,
                                    config_file=config_file,
                                    all_work_lp=ALL_WORK_LP, num_rounds=train_loop.num_rounds, global_step=train_loop.global_step,
                                    **EPISODE_RUNNER_PARAMS) as sim_round_queue:
