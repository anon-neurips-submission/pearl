...
from autograph.lib.mcts_chess import MCTS
from autograph.lib.running_ensemble_chess import get_parallel_queue, RandomReplayTrainingLoop, \
    run_episode_generic, run_episode_generic_lp
...
from autograph.net.mine_constructors import minenet_v1, mine_obs_rewriter_creator, minernd_v1, \
    mine_mazenet_v1, mine_obs_chess_rewriter_creator, mine_mazenet_v1_chess
...
if __name__ == '__main__':
    ...
    if EPISODE_RUNNER_TYPE == "chess_mcts_LP" or EPISODE_RUNNER_TYPE == "chess_mcts_noLP":
        LOSS_FUNC = "MCTS"
...
def run_chess_mcts_ensemble_LP(nets: List[torch.nn.Module], env: chessEnv, max_length: int,
    ...
    mcts = MCTS(env.env.env.env.env.action_spaces['player_0'].n, None, state_evaluator, None, **kwargs)
...
def run_chess_mcts_ensemble_noLP(nets: List[torch.nn.Module], env: chessEnv, max_length: int,
                         max_len_reward: Union[int, None],
                         device, train_state_rewriter: Callable[[Any], torch.Tensor],
...
def run_chess_ppo_noLP(nets: List[torch.nn.Module], env: AutShapingWrapper, max_length: int,
                         max_len_reward: Union[int, None],
                         device, train_state_rewriter: Callable[[Any], torch.Tensor],
                         state_observer: Callable[[Any], None] = None,lp_gen=False,
...
episode_runners = {
    "chess_noLP_ppo": run_chess_ppo_noLP,
    "chess_LP_ppo": run_chess_ppo_noLP,
    "chess_mcts_LP": run_chess_mcts_ensemble_LP,
    "chess_mcts_noLP": run_chess_mcts_ensemble_LP,
}
...
def run():
    ...
    for tr_idx in range(0, NUM_RUNS):
        ...
        if DEBUG_LP:
            confusing_input_test = get_confusing_input_pool(net, env, config)
        ...
        if EPISODE_RUNNER_TYPE == "chess_LP_ppo" or EPISODE_RUNNER_TYPE == "chess_mcts_LP":
            lp_gen = True
        else:
            lp_gen = False
        #TODO hardcode both _nets

        print('LP GEN IS', lp_gen)
